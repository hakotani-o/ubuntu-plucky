...
if [ ! -f ../overlay/tmp_var.txt ]; then
	echo "../overlay/tmp_var.txt not found"
	exit 1
fi
...
# 解凍
mkdir -p temp_deb
cp ../kernel/linux-image-${tmp_var}_${secnd}.deb .
...
# 再パッケージ
#dpkg-deb -b temp_deb linux-image-${tmp_var}-custom.deb
#cp linux-image-${tmp_var}-custom.deb ../kernel
rm linux-image-${tmp_var}_${secnd}.deb
dpkg-deb -b temp_deb linux-image-${tmp_var}_${secnd}.deb
cp linux-image-${tmp_var}_${secnd}.deb ../kernel
